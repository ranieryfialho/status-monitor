    ...
    <Card className="h-full border-none shadow-lg bg-card rounded-[20px] flex flex-col">
      <CardHeader className="px-6 py-4 border-b border-border/50 flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-lg font-bold text-foreground">Sessões do Site</CardTitle>
        </div>
        <Badge variant="destructive" className="bg-red-500/10 text-red-500 hover:bg-red-500/20">-8.3%</Badge>
      </CardHeader>
      <div className="flex-1 p-4 w-full min-h-[300px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            />
            <Area type="monotone" dataKey="anterior" name="Período Anterior" stroke="#94a3b8" fillOpacity={1} fill="url(#colorAnterior)" />
            <Area type="monotone" dataKey="atual" name="Este Período" stroke="hsl(var(--primary))" fillOpacity={1} fill="url(#colorAtual)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </Card>
